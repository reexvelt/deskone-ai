...
interface StoreState {
  ...
  createMissionDraft: (title: string) => Mission;
...
const seedProjects = (): Project[] => [
  { id: "p1", name: "Ebook Launch", description: "AI Execution Playbook", missionCount: 4, updatedAt: Date.now() - 86400000, color: "#3B82F6" },
  { id: "p2", name: "Content Engine", description: "YouTube + newsletter", missionCount: 7, updatedAt: Date.now() - 3600000 * 5, color: "#7C5CFF" },
  { id: "p3", name: "Growth Ops", description: "Outbound & campaigns", missionCount: 3, updatedAt: Date.now() - 3600000 * 30, color: "#22C55E" },
];
...
const seedMissions = (): Mission[] => {
  ...
  return [
    {
      id: "m1",
      title: "Launch my ebook",
      objective: m1.objective,
      status: "running",
      progress: 42,
      createdAt: Date.now() - 3600000 * 6,
      startedAt: Date.now() - 3600000 * 5,
      estimatedMinutes: m1.minutes,
      apps: m1.apps,
      steps: m1.steps.map((s, i) => ({ ...s, status: i < 2 ? "done" : i === 2 ? "running" : "pending" })),
      logs: [
    ...
    },
    {
      ...
      estimatedMinutes: m2.minutes,
      apps: m2.apps,
      steps: m2.steps.map((s) => ({ ...s, status: "done" })),
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(KEY, JSON.stringify({ missions, projects, integrations, models }));
  }, [missions, projects, integrations, models]);
  ...
  const value: StoreState = useMemo(
    ...
    [missions, projects, integrations, models, credits],
  );
